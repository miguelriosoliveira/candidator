...
describe("CandidateList", () => {
	...
	it("renders loading state initially", () => {
		...
		vi.mocked(candidateRepository.getCandidates).mockReturnValue(
			new Promise(() => {}),
		);
	...
	it("renders error state when the API fails", async () => {
		vi.mocked(candidateRepository.getCandidates).mockRejectedValue(
	...
	it("renders candidates successfully", async () => {
		...
		vi.mocked(candidateRepository.getCandidates).mockResolvedValue(
			mockResponse,
		);
		...
		await waitFor(() => {
			expect(
				screen.queryByText("Loading candidates..."),
